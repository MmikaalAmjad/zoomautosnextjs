...
//         toDate: '',
//         toTime: 'ANYTIME',
//         multidropoff:  [{ vehicleRegistration: "" ,carType:"Used Car"},{ vehicleRegistration: "" ,carType:"Used Car"}], // Initial empty vehicle
//         notes: '',
//         releaseNotes: '',
...
const [formData, setFormData] = useState<Record<string, any>>({
  ...
  const [jobId, setJobId] = useState('');//rm login failed 
  ...
  const updatedVehicles = [...formData.multidropoff] as any[]; // <-- tell TS this is an array of objects
  updatedVehicles[index] = { ...(updatedVehicles[index] as any), [name]: value };
  setFormData((prevData) => ({
    ...prevData,
    multidropoff: updatedVehicles,
  }));
...
<option value="ANY" disabled>ANY</option>
  ...
  <button
    ...
    type="button"
    className={`buttonb ${
  formData.multidropoff?.[index]?.carType === button ? "active" : ""
}`}
    ...
    {(errors.vehicles as any)[index].manufacturer.message}
      ...
      <div className="form-group">
        ...
        <input
          ...
          onChange={(e) => handleVehicleChange(index, e)} // Handle onChange for dynamic vehicles
        />
        {(errors.vehicles as any)?.[index]?.model && (
  <p className="error-message">
    {(errors.vehicles as any)[index].model.message}
  </p>
)}
      </div>
...
export default MultidropoffVehicleForm;
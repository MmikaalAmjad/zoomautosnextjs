...
// Updated function for handling vehicle form changes
const handleVehicleChange = (index:any, e:any) => {
  const { name, value } = e.target;
  
  // Ensure the vehicles state is properly updated
...
// Updated function for handling return vehicle form changes
const handlereturnVehicleChange = (index:any, e:any) => {
  const { name, value } = e.target;

  // Ensure the returnVehicles state is properly updated
...
const { dealerDetails, updateDealerDetails } = useDealerAdmin();
  ...
  // Restore dealer details from localStorage
  useEffect(() => {
    const storedUser = localStorage.getItem('DealerData');
    if (storedUser) {
      const user = JSON.parse(storedUser);
      updateDealerDetails(user);
    }
  }, [updateDealerDetails]);

  // Fetch existing job IDs and generate a unique job ID
...
const handleChange = (index:any, e:any) => {
  ...
  const [returnvehicleerrors, setreturnvehicleErrors] = useState<any>({}); // Store errors per index
  
    const handlereturnEnquiry = async (index:any, registrationNumber:any) => {
      try {
        // Clear error for this specific index before request
        setreturnvehicleErrors((prevErrors:any) => ({ ...prevErrors, [index]: null }));
    ...
    if (error.message.includes("404")) {
      errorMessage = "Vehicle not found. Please check the registration number or Enter Data Manually.";
        ...
        setreturnvehicleErrors((prevErrors:any) => ({
          ...prevErrors,
          [index]: errorMessage,
        }));
      }
    ...
    const [vehicleerrors, setvehicleErrors] = useState<any>({}); // Store errors per index
  
  const handleEnquiry = async (index:any, registrationNumber:any) => {
    try {
      setErrors((prevErrors:any) => ({ ...prevErrors, [index]: null })); // Clear error for this index
    ...
    if (error.message.includes("404")) {
      ...
      setvehicleErrors((prevErrors:any) => ({ ...prevErrors, [index]: errorMessage }));
    }
  };
  ...
  formData.vehicles.some((vehicle:any) =>
    formData.returnVehicles.some(
      (returnVehicle:any) =>
        returnVehicle.vehicleRegistration === vehicle.vehicleRegistration || vehicle.vin && returnVehicle.vin && vehicle.vin === returnVehicle.vin
    )
  );
...
if (hasDuplicate) {
  ...
  return (
    ...
    <p>A car is taken from the collection point and delivered to its delivery point. From delivery point taking another vehicle and returned to the collection point.
    </p>
      <p>
...
/>
    ...
    {index >= 1 && (
      ...
      <button
        type="button"
        className="contact-button"
        onClick={() => removeReturnVehicleForm(form.id)}
      >
        Remove
      </button>
      </div>
...
<div className='form-group'>
<button type="button" className="contact-button" onClick={addVehicleAndReturnForm}>
        Add Another Return Vehicle</button>
      </div>
      <h2>Collection Details</h2>
      <div className='form-row'>
      <div className='form-group'>
...
export default RegisteredReturnJobform;

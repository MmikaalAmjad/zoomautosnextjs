...
const RecordEditFormActive = () => {
  const { jobId } = useParams(); // Get the jobId from the URL

  const [record, setRecord] = useState(null);
  const handleAdditionalCostChange = (e:any) => {
    let value = e.target.value.replace("£", "").trim(); // Remove existing £ to prevent duplication
    ...
    setFormData((prevData) => ({
      ...prevData,
      additionalCost: value, // Store only the number
    }));
  };
...
<div className="form-group">
            ...
            <input
  type="text" // Change to text to allow £ symbol
  name="additionalCost"
  value={formData.additionalCost ? `£ ${formData.additionalCost}` : "£ "}
  onChange={handleAdditionalCostChange}
  placeholder="£ 0.00"
/>


            <label>Additional Cost Reason:</label>
            <input
              type="text"
...
export default RecordEditFormActive;